export default {
  ...
  settingTabs: {
    ...
    notification: {
      title: '通知',
      description: '企业微信、Telegram 通知渠道与消息发送范围',
    },
    about: {
      title: '关于',
      description: '软件版本',
  ...
  setting: {
    ...
    notification: {
      ...
      invalidConfigTitle: '部分历史通知配置已失效',
      invalidConfigHint: '请打开对应渠道补齐或修正配置后保存；无法识别的历史项会在保存时移除。',
      ...
      testFailed: '测试通知发送失败，请检查渠道配置和接收目标',
      channel: '通知',
      wechat: '企业微信',
      resourceDownload: '资源下载',
      mediaImport: '整理入库',
    ...
    words: {
      ...
      syncUrl: '远程同步链接',
      syncUrlPlaceholder: 'https://github.com/owner/repo(自动识别 Words/ 下 4 个词表文件)',
      syncEnabled: '自动同步',
      syncIntervalDays: '频率(天)',
      syncSaveSettings: '保存设置',
      syncNow: '立即同步',
      syncNever: '从未同步',
      syncStatusSuccess: '成功',
      syncStatusPartial: '部分失败',
      syncSuccess: '词表同步完成:{message}',
      syncFailed: '词表同步失败,请检查链接与网络',
      syncSettingsSaved: '同步设置已保存',
      syncSettingsSaveFailed: '同步设置保存失败!',

      syncTitle: '远程同步',
      syncShort: '远程同步',
      syncDesc: '从远程仓库链接同步上方 4 类词表配置',
      syncEveryDays: '每 {days} 天',
      syncAutoOff: '已关闭',
      syncLastSync: '上次同步',
      syncFileHint:
        '链接指向的仓库需包含 Words/ 目录,其中 4 个文件分别对应 4 类词表:CustomIdentifiers.txt(识别词)、CustomReleaseGroups.txt(制作组)、Customization.txt(占位符)、TransferExcludeWords.txt(屏蔽词)。',
      syncAddSource: '添加同步源',
      syncAll: '全部同步',
      syncNoSource: '尚未配置同步源,点击上方按钮添加',
      syncTables: '同步词表',
      syncStatusSkipped: '无启用词表,已跳过',
      syncSourceRemoved: '同步源已删除',
      syncRemoteSummary: '远程同步 {count} 条 · 来自 {sources} 个源(与上方本地词表追加生效)',
      syncSourcesSummary: '{total} 个源 · {enabled} 启用',
    },
    search: {
      basicSettings: '基础设置',
    ...
    category: {
      ...
      advanced: '进阶',
      advancedHint: '直接编辑 category.yaml 原文,保存保留注释;从可视化页保存将移除所有注释',
      restoreTemplate: '恢复默认模板',
      restoreConfirmTitle: '恢复默认模板',
      restoreConfirmMessage: '将使用默认模板覆盖当前编辑内容,尚未保存的修改会丢失,确定继续吗?',
      rawLoadFailed: '加载配置原文失败: {message}',
      rawSaveSuccess: '配置原文保存成功,注释已保留',
      discardRawChangesTitle: '放弃未保存的原文修改？',
      discardRawChangesMessage: '可视化配置已更新，重新加载原文将丢失当前未保存的修改，确定继续吗？',
    },
    rule: {
      customRules: '自定义规则',
  ...
  dialog: {
    ...
    subscribeEdit: {
      ...
      skipLibraryCheck: '不检测媒体库存在集',
      skipLibraryCheckHint: '媒体库已有集也照常搜索下载',
      ...
      releaseGroup: '字幕组',
      releaseGroupHint: '匹配种子制作组，支持正则表达式，留空不限',
      filterGroups: '优先级规则组',
      ...
      versionNotLoaded: '订阅数据尚未加载完成，请稍候再保存',
      versionEnabled: '启用版本',
      versionLastCannotDelete: '至少保留一个版本',
    },
    subscribeVersions: {
      title: '选择订阅版本',
      addVersion: '新增版本',
      completed: '已完成',
      deleteConfirm: '确定删除版本「{name}」吗？正在下载的任务和已有文件不受影响。',
      editVersionHint: '编辑此版本',
      disabled: '已停用',
      noVersions: '暂无订阅版本',
  ...
  profile: {
    ...
    confirmToChangePassword: '修改密码前需要验证当前登录密码。',
  ...
  transferHistory: {
    ...
    clearConfirmTitle: '清空整理记录',
    clearConfirmContent: '将清空全部整理记录，与当前搜索/筛选无关。仅删除记录，不影响已整理的文件，且不可撤销。',
    clearSuccess: '已清空 {count} 条整理记录',
  ...
  downloader: {
    ...
    btDefaultChanged: '存在 BT 站点默认下载器，已替换',
    hostRequired: '地址不能为空',
  ...
  renameFormat: {
    modeSimple: '简易',
    modeAdvanced: '进阶',
    emptyHint: '点击下方字段或输入文本开始拼接命名格式',
    customTextPlaceholder: '自定义文本（如 /、-、空格）',
    insertText: '插入文本',
    reset: '重置默认',
    ...
    fieldTitle: '标题',
    fieldYear: '年份',
    fieldTitleYear: '标题+年份',
    fieldSeason: '季号(01)',
    fieldSeasonFmt: '季号(S01)',
    fieldEpisode: '集号(01)',
    fieldSeasonEpisode: '季集(S01E01)',
    fieldEpisodeTitle: '集标题',
    fieldPart: 'Part',
    fieldVideoFormat: '分辨率',
    fieldVideoCodec: '视频编码',
    fieldAudioCodec: '音频编码',
    fieldEdition: '版本',
    fieldReleaseGroup: '制作组',
